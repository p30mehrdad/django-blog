...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for name in category_list:
            Category.objects.get_or_create(name=name) # get or create tekrari import nemikone
            
        for _ in range(10): # make 10 post
            Post.objects.create(
                author = profile,
                # image = "images/test.jpg"
                title = self.fake.paragraph(nb_sentences=1),
                content=self.fake.paragraph(nb_sentences=10) ,
                status= True, # status = random.choice([True,False])
